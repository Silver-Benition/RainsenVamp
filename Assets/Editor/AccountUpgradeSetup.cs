...
public static class AccountUpgradeSetup
{
    ...
    public static void Build()
    {
        ...
        shop.StatusText = Text("ShopStatus", details, "", 0.13f, 0.19f, 0.64f, 0.39f, 17);
        shop.StatusText.alignment = TextAlignmentOptions.TopLeft;
        shop.BuyButton = Button("ShopBuy", details, "购买", 0.66f, 0.05f, 0.81f, 0.29f);
        shop.RefundButton = Button("ShopRefund", details, "退款", 0.825f, 0.05f, 0.98f, 0.29f);
        ...
        RectTransform toggleRoot = Box("ShopEnabled", details, 0.13f, 0.025f, 0.62f, 0.17f, Color.clear);
        Toggle toggle = toggleRoot.gameObject.AddComponent<Toggle>();
        toggle.toggleTransition = Toggle.ToggleTransition.None;
        RectTransform toggleBox = Box("Box", toggleRoot, 0, 0.15f, 0, 0.85f, new Color32(45, 71, 87, 255));
        toggleBox.sizeDelta = new Vector2(24, 0);
        Border(toggleBox, new Color32(180, 203, 210, 255));
        Image checkmark = Box("Checkmark", toggleBox, 0.2f, 0.2f, 0.8f, 0.8f, new Color32(36, 219, 242, 255)).GetComponent<Image>();
        checkmark.raycastTarget = false;
        toggle.targetGraphic = toggleBox.GetComponent<Image>();
        toggle.graphic = checkmark;
        toggle.isOn = true;
        Text("Label", toggleRoot, "启用（下局生效）", 0.065f, 0, 1, 1, 18);
        toggleRoot.gameObject.AddComponent<AccountShopCancelRelay>().Bind(shop);
        shop.EnabledToggle = toggle;
        toggleRoot.gameObject.SetActive(false);
